...
一个**不碰源文件、手工入库、集中管理**的本地文件/文件夹管理工具。

> 你的文件和文件夹散落在硬盘各处？这个工具帮你把它们"登记在册"，随时查找、打开、定位——不移动、不复制、不改动任何源文件。
...
- Python 3.8+
- Flask >= 3.0 + pywebview >= 5.0 + pypinyin >= 0.50 + natsort >= 8.4
...
```bash
# 1. 安装依赖
pip install flask pywebview pypinyin natsort
...
双击 `打包.bat`，完成后在当前目录得到 `file-warehouse.exe`，可拷贝至任意目录运行。
...
| POST | `/api/browse-files` | 打开原生文件/文件夹选择对话框 |
...
| is_folder | INTEGER | 是否为文件夹：0=文件, 1=文件夹 |

---

## 设计说明

- **零侵入**：不修改、不移动源文件/文件夹，仅记录路径
- **文件夹支持**：可将文件夹登记入库，作为快捷链接直达目录
- **纯本地**：无需网络连接，数据完全本地存储
